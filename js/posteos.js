...
// contenedor de los datos de usuarios modal
let contenedorAvatarUser = document.querySelector("#userLogueado");
...
//capturamos el modal que usamos para mostrar el usuario en tamaños pequeños
let myModalUser = new bootstrap.Modal(document.getElementById("modalUser"), {
  keyboard: false,
});
...
contenedorAvatarUser.innerHTML = estructura_avatar;
...
const crearCards = function (array) {
  ...
  if (posteosActivos.length === 0) {
    return (contenedor_cards.innerHTML = `<h4 class=" text-center text-white">No se encontraron resultados para la búsqueda.</h4>`);
  }
  //----------------------------------------------------
  // array.map(function (item) {
  posteosActivos.map(function (item) {
    let card = document.createElement("div");
    card.classList = "card mb-3 bg-dark text-white";
           ...
           <div class="card-header">${item.usuario}</div>
              ...
              <div class="card-footer">
                  ...
                  <textarea id="textarea${
                    item.id
                  }" class="form-control" rows="1" placeholder="Comentario..."></textarea>
    ...
    let contenido_card2 = `
        ...
        <div class="card-footer d-flex align-items-center">
          ...
          <button type="button" class="btn btn-primary" onclick="crearComentario(${
            item.id
          })">Publicar</button>
...
//Si hacemos click en el boton para ver usuario
...
document.querySelector("#logoutModal").addEventListener("click", function () {
  localStorage.removeItem("usuario");
  location.href = "../index.html";
});
// -------------------------------------------------------------------
